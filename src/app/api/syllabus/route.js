...
export async function POST(req) {
  try {
    ...
    const adminMailOptions = {
      ...
      to: adminEmail,
      subject: `New Syllabus Request: ${course} - ${name}`,
      html: `
        <!DOCTYPE html>
        <html>
          ...
          <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
            ...
            <div style="padding: 40px 30px;">
              <div style="color: #1a202c; font-size: 24px; font-weight: bold; margin-bottom: 20px; border-bottom: 2px solid #edf2f7; padding-bottom: 15px;">
                New Syllabus Request
              </div>
              
              <table style="width: 100%; text-align: left; border-collapse: collapse; font-size: 16px;">
                <tr>
                  <th style="padding: 12px 0; color: #4a5568; font-weight: 600; border-bottom: 1px solid #e2e8f0; width: 35%;">Name</th>
              ...
              <div style="margin-top: 25px; font-size: 14px; color: #4a5568;">
                <strong>Action Required:</strong> Please email the requested syllabus PDF to the student.
              </div>
            </div>
    ...
    const userMailOptions = {
      from: `"PetroLabs Training" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Syllabus Request Received: ${course}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"></head>
        <body style="font-family: Arial, sans-serif; background-color: #f4f7fa; margin: 0; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
            ...
            <div style="padding: 40px 30px; color: #4a5568; line-height: 1.6; font-size: 16px;">
              ...
              <p style="margin: 0 0 15px 0;">Thank you for your interest in the <strong style="color: ${brandColor};">${course}</strong> training program.</p>
              <p style="margin: 0 0 15px 0;">We have received your request for the course syllabus. Our team will email you the detailed curriculum shortly.</p>
              
              <div style="text-align: center; margin-top: 35px; margin-bottom: 35px;">
                <a href="https://petrolabsindia.com/training" style="display: inline-block; background-color: ${brandColor}; color: #ffffff; text-decoration: none; padding: 14px 28px; border-radius: 6px; font-weight: bold; font-size: 16px;">Explore All Programs</a>
              </div>
